...
func TestTemplate_ExecuteAlgorithm(t *testing.T) {
	...
	t.Run("Using anonymous functions adapted to an interface with nil function", func(t *testing.T) {
		messageRetriever := MessageRetrieverAdapter(nil)
		if messageRetriever == nil {
			t.Fatal("Can not continue with a nil MessageRetriever")
		}
		template := Template{}
		res := template.ExecuteAlgorithm(messageRetriever)
		expectedOrError(res, "  ", t)
	})
}
